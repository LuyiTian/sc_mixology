...
## linnorm+DrImute
```{r}
imp <- DrImpute(assay(sce79.3_qc, "linnorm"))
assay(sce79.3_qc, "linnorm_DrImpute") <- imp
```
...
imp <- DrImpute(assay(sce79.3_qc, "DESeq"))
assay(sce79.3_qc, "DESeq_DrImpute") <- imp
})
...
time_DESeq_SAVER <- system.time({
  ...
  assay(sce79.3_qc, "DESeq_SAVER") <- log2(saver_out$estimate + 1)
})
```
...
imp <- DrImpute(assay(sce79.3_qc, "CPM"))
assay(sce79.3_qc, "CPM_DrImpute") <- imp
...
time_DESeq_SAVER <- system.time({
  ...
  assay(sce79.3_qc, "cpm") = cpm(counts(sce79.3_qc))
  registerDoParallel(cores = 64)
  saver_out <- saver(assay(sce79.3_qc, "cpm"), size.factor = 1)
  #load("saver_out.RData")
  assay(sce79.3_qc, "CPM_SAVER") <- log2(saver_out$estimate + 1)
})
```
...
```{r}
PCA_DESeq_DrImpute <- runPCA(sce79.3_qc, exprs_values = "DESeq_DrImpute", ncomponents = 2)
PCA_DESeq_SAVER <- runPCA(sce79.3_qc, exprs_values = "DESeq_SAVER")
...
```{r}
PCA_CPM_DrImpute <- runPCA(sce79.3_qc, exprs_values = "CPM_DrImpute", ncomponents = 2)
PCA_CPM_SAVER <- runPCA(sce79.3_qc, exprs_values = "CPM_SAVER")
...
```{r}
PCA_linnorm_DrImpute <- runPCA(sce79.3_qc, exprs_values = "linnorm_DrImpute", ncomponents = 2)
...
```{r}
write.csv(sil_all, file = "imputation/9cell_sil.csv")
write.csv(f, file = "imputation/9cell_f1.csv")
write.csv(f2, file = "imputation/9cell_f2.csv")
```