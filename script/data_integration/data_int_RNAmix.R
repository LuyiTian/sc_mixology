...
log_file =paste("log_file/data_int_RNAmix",format(Sys.time(), "%a_%b_%d"),"txt",sep = ".")
cat(paste(format(Sys.time(), "%a %b %d %X %Y"), "start preprocessing...\n"), file = log_file, append = TRUE)

res2 = readRDS("/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/final/RNAmix_all_after_imputation.Rds")
...
for (vnorm in as.character(unique(res2$norm_method))) {
  for(vimp in as.character(unique(res2$impute_method))){
    ...
    res_df = data.frame(data="RNAmix",norm_method=vnorm,impute_method=vimp)
...
for(i in 1:9){
  saveRDS(res_int[(4*(i-1)+1):(4*i),],file=paste0("/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/RNAmix_int_data_",i ,".Rds"))
}
...
saveRDS(res_t, file="/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/RNAmix_after_data_int.Rds")
