...
```{r}
library(zinbwave)
sce_all = SingleCellExperiment(assay=list(counts=cbind(counts(sce10x_qc)[high_var_genes_10000,], counts(sce4_qc)[high_var_genes_10000,], counts(scedrop_qc_qc)[high_var_genes_10000,])))
sce_all$protocol = as.factor(anno_col)
sce_all$cell_line = as.factor(anno_cell_line)

sce_all_zw =  zinbwave(sce_all, 
                       X="~protocol", 
                       K=2, 
...
```{r}
ggplot(data=NULL,aes(x=reducedDim(sce_all_zw,"zinbwave")[,1],y=reducedDim(sce_all_zw,"zinbwave")[,2],shape=sce_all$protocol,col=sce_all$cell_line))+geom_point()
```


```{r}
ggplot(data=NULL,aes(x=reducedDim(sce_all_zw,"zinbwave")[,1],y=reducedDim(sce_all_zw,"zinbwave")[,2],col=sce_all$protocol,shape=sce_all$cell_line))+geom_point()
```
