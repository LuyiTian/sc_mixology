...
MAX_TIME = 60*120
log_file =paste("log_file/trajectory_RNAmix",format(Sys.time(), "%a_%b_%d"),"txt",sep = ".")
...
clean_sce = function(sce){ # remove raw expr data after clustering to save space
  counts(sce) = NULL
  return(sce)
}
...
saveRDS(res_c, file="/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/final/RNAmix_after_trajectory.Rds")
