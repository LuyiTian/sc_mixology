...
log_file =paste("log_file/clustering_sc_all",format(Sys.time(), "%a_%b_%d"),"txt",sep = ".")
...
format_rowname = function(sce){
  rownames(sce) = paste("XXXX",rownames(sce),"ENSGXXXX",sep="_")
  return(sce)
}
...
```{r}
saveRDS(res_c, file="/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/sc_all_after_clustering.Rds")
```

```{r}
res2 = res2[order(res2$norm_method),]

for (i in 1:30){
  saveRDS(res2[(8*(i-1)+1):(8*i),],paste0("/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/for_mac/sc_after_imp_",i ,".Rds"))
}
```
