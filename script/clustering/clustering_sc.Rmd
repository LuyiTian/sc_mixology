...
log_file =paste("log_file/clustering_sc",format(Sys.time(), "%a_%b_%d"),".txt",sep = ".")
...
```{r}
res2 = readRDS("/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/final/sc_after_imputation.Rds")
```
...
```{r}
saveRDS(res_c, file="/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/final/sc_after_clustering.Rds")
```
...
```{r}
cat(paste(format(Sys.time(), "%a %b %d %X %Y"), "SC Done! saved the result to file.\n"), file = log_file, append = TRUE)
```
