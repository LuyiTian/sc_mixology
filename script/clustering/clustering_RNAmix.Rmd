...
log_file =paste("log_file/clustering_RNAmix",format(Sys.time(), "%a_%b_%d"),".txt",sep = ".")
...
```{r}
saveRDS(res_c, file="/stornext/General/data/user_managed/grpu_mritchie_1/SCmixology/analysis_for_resubmit/rdata/RNAmix_all_after_clustering.Rds")
```
...
```{r}
cat(paste(format(Sys.time(), "%a %b %d %X %Y"), "RNAmix Done! saved the result to file.\n"), file = log_file, append = TRUE)
```
